...
def show_feature_map():
    cat = plt.imread('../DogVsCat/test_data/tiger.jpeg')  # unit8
    plt.imshow(cat)
    cat = tf.cast(cat, tf.float32)  # [360, 300, 3]
    x = tf.reshape(cat, [1, 304, 166, 3])  # [1, 360, 300, 3]
    ...
    feature_map = tf.reshape(x_w, [304, 166, out])
...
def show_rich_feature():
    ...
    with tf.variable_scope('conv1_1', reuse=False):
        ...
        sess = tf.Session()
        sess.run(tf.global_variables_initializer())
        feature_map = tf.reshape(x_relu, [304, 166, out])
        images = tf.image.convert_image_dtype(feature_map, dtype=tf.uint8)
        images = sess.run(images)
        print(images.shape)
        plt.figure(figsize=(10, 10))
