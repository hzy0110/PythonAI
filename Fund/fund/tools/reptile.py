...
class Reptile:
    ...
    def get_all_manage(self):
        ...
        for manager in rawData['data']:
            ...
            # compName = manage[3]
            fund_code_list = manager[4]
            fundName = manager[5]
            total_work_day = manager[6]
            current_fund_best_report_per = manager[7]
            current_fund_best_report_code = manager[8]
            current_fund_money = manager[10]
            all_fund_best_report_per = manager[11]
            allManage.append([
            ...
            manager_info_pd = pd.DataFrame(allManage, columns=[
                'manager_code', 'manager_name', 'company_code', 'total_work_day',
                'current_fund_best_report_per', 'current_fund_best_report_code', 'current_fund_money', 'all_fund_best_report_per'
            ])
            manager_currnet_pd = pd.DataFrame()
            if len(fund_code_list) > 0:
                manager_currnet_pd = pd.DataFrame(fund_code_list, columns=['fund_code'])
                manager_currnet_pd = manager_currnet_pd.T
                manager_currnet_pd.insert(0, 'manager_code', manager_code)

        return manager_info_pd, manager_currnet_pd
