...
class Tool:
    ...
    def get_pingzhongdata_2_df(self, code, basic_info_pd, funds):
        # 数据合并到DF
    ...
    def replace_none(self, none_str):
        ...
        #         index=False)
        # to
        # rateInSimilar_pd.to_csv(
        #     './rateInSimilar_pd' + mid + '.csv',
        #     encoding='utf_8_sig',
        #     mode='a',
        #     header=False)
        # grandTotal_pd.to_csv(
        #     './grandTotal_pd' + mid + '.csv',
        #     encoding='utf_8_sig',
        #     mode='a',
        #     header=False)
        # worth_pd.to_csv(
    ...
    #
    #     netWorthDF = pd.DataFrame(
    #         funds[0], columns=['时间戳', '单位净值', '净值回报', '每份派送金'])
    #     ACWorthDF = pd.DataFrame(funds[1], columns=['时间戳', '累计净值'])
    #     grandTotalDF = pd.DataFrame(funds[2], columns=['累计收益率名称', '累计收益率信息'])
    #     for i in range(len(grandTotalDF['累计收益率信息'])):
    #         grandTotalDF['累计收益率信息'][i] = pd.DataFrame(
    #             grandTotalDF['累计收益率信息'][i],
    ...
    #
    #     rateInSimilarTypeDF = pd.DataFrame(funds[3], columns=['时间戳', '同类排名数', 'sc'])
    #     rateInSimilarPersentDF = pd.DataFrame(funds[4], columns=['同类排名百分比'])
    #     # fluctuationScaleDF = pd.DataFrame(
    #     #     funds[7], columns=['规模变动日期', "规模变动净资产亿元", "规模变动较上期环比"])
    #
    #     # 已平
    ...
    #     grandTotal_pd.insert(0, 'code', code)
    #
    #     return fund_wide_pd, rateInSimilar_pd, grandTotal_pd, worth_pd
    #
    #     # if mid == "1":
    #     #     fund_wide_pd.to_csv(
    #     #         './fund_wide_pd' + mid + '.csv',
