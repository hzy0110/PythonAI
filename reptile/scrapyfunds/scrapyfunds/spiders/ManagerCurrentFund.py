...
class FundsSpider(scrapy.Spider):
    ...
    name = 'ManagerCurrentFund'  # 唯一，用于区别Spider。运行爬虫时，就要使用该名字
    ...
    def start_requests(self):
        ...
        # request = scrapy.Request(url_test, callback=self.parse_funds_list, meta={'manager_code': '30395964'})
        ...
        for manager_code in allManager_id_s:
            request = scrapy.Request(url.format(manager_code), callback=self.parse_funds_list, meta={'manager_code': manager_code})
            requests.append(request)
        return requests

    def parse_funds_list(self, response):
        manager_code = response.meta['manager_code']
        datas = response.xpath('//table[@class="ftrs"]')
        # manages_list = []
        data = datas[1]
        # for data in datas:
        data_thead_r = data.xpath('thead/tr/th/text()').getall()
        manager_current_pd = pd.DataFrame(columns=data_thead_r)
        ...
        for body_tr in data_tbody:
            ...
            manager_current_pd = manager_current_pd.append(manage_body_pd, ignore_index=True)
        # print(manage_pd)
        manager_current_pd.insert(0, 'manager_code', manager_code)
        # print('manage_pd', manager_current_pd.columns.tolist())
        # manages_list.append(manage_pd)
        # 对于Selector类型的对象，并不能使用extract_first()方法，而使用get()可以
        # manager_current_pd = manages_list[0]
        # manager_current_pd = manages_list[1]
        manager_current_pd = manager_current_pd.loc[:, ['manager_code', '基金代码']]
        manager_current_pd.columns = ['manager_code', 'fund_code']
        # print('manage_pd', manager_current_pd.columns.tolist())
        fundsItems = []
        fundsItem = ScrapyfundsItem()
        fundsItem['manager_current_pd'] = manager_current_pd
