...
class FundsSpider(scrapy.Spider):
    # 抓取基金评级，季度级
    name = 'FundsSHZQRating'  # 唯一，用于区别Spider。运行爬虫时，就要使用该名字
    ...
    def start_requests(self):
        if os.path.exists('./data/fund/fund_shzq_rating_pd.csv'):
            os.remove('./data/fund/fund_shzq_rating_pd.csv')
    ...
    def parse_funds_list(self, response):
        data = response.xpath('//table[@class="table3"]')
        ...
        for th in data_thead_th:
            ...
            if c_name == '代码':
                c_name = 'fund_code'
            if c_name == '3年期评级':
                c_name = 'shzq_start_3y_num'
            if c_name == '5年期评级':
                c_name = 'shzq_start_5y_num'
            if c_name == 'fund_code' or c_name == 'shzq_start_3y_num' or c_name == 'shzq_start_5y_num':
        ...
        fundsItem['fund_shzq_rating_pd'] = fund_rating_pd
