...
class FundsSpider(scrapy.Spider):
    # 抓取基金评级
    name = 'FundsJAJXRating'  # 唯一，用于区别Spider。运行爬虫时，就要使用该名字
    ...
    def start_requests(self):
        if os.path.exists('./data/fund/fund_jajx_rating_pd.csv'):
            os.remove('./data/fund/fund_jajx_rating_pd.csv')
    ...
    def parse_funds_list(self, response):
        data = response.xpath('//table[@class="table4"]')
        ...
        fundsItem['fund_jajx_rating_pd'] = fund_rating_pd
